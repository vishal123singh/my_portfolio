    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-cyan-400 text-lg font-semibold">Chat with Vishal’s Assistant</h3>
          <button onClick={onClose} className="text-slate-100 text-lg">✖</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 mb-4">
          {messages.map((msg, i) => (
            ...
            >
              <div
                className={`rounded-lg px-4 py-2 max-w-[80%] text-sm leading-relaxed ${msg.role === 'user' ? 'bg-slate-700 text-white' : 'bg-slate-900 text-slate-200'
                  }`}
              >
                {msg.role === 'assistant' ? (
